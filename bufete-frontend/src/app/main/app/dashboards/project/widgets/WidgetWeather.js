...
import { Icon, Typography, Paper, IconButton, Divider } from '@material-ui/core';
  ...
  return (
    <Paper className="w-full rounded-8 shadow-none border-1">
      ...
      <div className="flex items-center justify-center p-16 pb-32">
        <Icon className="meteocons text-40" color="action">
          {widget.locations[widget.currentLocation].icon}
        </Icon>
        <Typography className="text-44 ml-16" color="textSecondary">
          {widget.locations[widget.currentLocation].temp[widget.tempUnit]}
        </Typography>
        <Typography className="text-48 font-300 ml-8" color="textSecondary">
          °
        </Typography>
      ...
      <div className="flex justify-between items-center p-16">
        <div className="flex items-center">
          ...
          <Typography className="ml-4" color="textSecondary">
            {widget.speedUnit}
          </Typography>
        </div>
      ...
      <div className="w-full py-16">
        {widget.locations[widget.currentLocation].next3Days.map(day => (
          <div className="flex items-center justify-between w-full py-16 px-24" key={day.name}>
            ...
            <div className="flex items-center">
              <Icon className="meteocons text-24 mr-16" color="action">
                {day.icon}
              </Icon>
